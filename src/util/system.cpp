...
const char * const BITCOIN_CONF_FILENAME = "maximcoin.conf";
...
#else
    const char* pszModule = "maximcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Maximcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Maximcoin";
#else
    // Unix
    return pathRet / ".maximcoin";
#endif
#endif
}
